...
(function(Promise) {
	...
	function IALoader(canvas, game, callbacks, scale) {
		...
		if (/archive\.org$/.test(document.location.hostname)) {
			...
			images = {
				ia: img("images/other_logos/ia-logo-150x150.png"),
				mame: img("images/other_logos/mame.png"),
				mess: img("images/other_logos/mame.png"),
				dosbox: img("images/other_logos/dosbox.png"),
				sae: img("images/other_logos/sae.png"),
				pce: img("images/other_logos/pce.png"),
				vice: img("images/other_logos/vice.svg")
			};
		}
				...
				loading.then(function(data) {
					...
					.then(function(data) {
							...
							if (module && module.indexOf("dosbox") === 0) {
								...
								// noinspection JSUnresolvedFunction,JSUnresolvedVariable
								config_args.push(cfgr.model(modulecfg.driver), cfgr.rom(modulecfg.bios_filenames));
							} else if (module && module.indexOf("pce-") === 0) {
								// noinspection JSUnresolvedFunction,JSUnresolvedVariable
								config_args.push(cfgr.model(modulecfg.driver), cfgr.extraArgs(modulecfg.extra_args));
							} else if (module) { // MAME
								// noinspection JSUnresolvedFunction,JSUnresolvedVariable
		...
		function get_pce_files(cfgr, metadata, modulecfg, filelist) {
			...
			meta_props_matching(meta, /^pce_drive_([a-zA-Z0-9]+)$/).forEach(function(result) {
				var key = result[0];
				game_files_counter[meta[key]] = result[1][1];
			});
	...
	function Emulator(canvas, callbacks, loadFiles) {
		...
		function setupSplash(canvas, splash, globalOptions) {
			...
			if (!splash.splashElt) {
				...
				if (!globalOptions.hasCustomCSS) {
					...
					splash.splashElt.style.top = '22px';
		...
		var drawsplash = function() {
			...
			if (!splash.splashimg.src) {
				splash.splashimg.src = "images/logo/emularity_color_small.png";
			}
		};
	...
	function _SDL_CreateRGBSurfaceFrom(pixels, width, height, depth, pitch, rmask, gmask, bmask, amask) {
		...
		for (var pixelOffset = 0; pixelOffset < width * height; pixelOffset++) {
			surfacePixelData[pixelOffset * 4] = HEAPU8[pixels + (pixelOffset * channels)]; // R
