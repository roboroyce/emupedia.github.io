...
Wolf.PlayerAI = (function() {
	...
	function controlPlayer(game, level, player, tics) {
		...
		if (player.ammo[Wolf.AMMO_BULLETS] === 0) {
			checkTile = function(x, y) {
				// noinspection JSBitwiseOperatorUsage
				return !(level.tileMap[x][y] & Wolf.BLOCK_TILE);
			};
		}
		...
		var isBoss = function(actor) {
			return actor.type === Wolf.en_boss || actor.type === Wolf.en_mecha || actor.type === Wolf.en_schabbs || actor.type === Wolf.en_hitler
	...
	function drawMinimap(level, player, closest) {
		var canvaselement = null;
		var canvas = null;

		if ($('#minimap').length > 0) {
			canvaselement = $('#minimap')[0];
		...
		for (var n = 0; n < level.state.numGuards; ++n) {
			...
			if (guard.flags & Wolf.FL_SHOOTABLE) {
				if (guard == closest)
					drawRect(canvas, guard.tile.x, guard.tile.y, "red");
				else
					drawRect(canvas, guard.tile.x, guard.tile.y, "darkred");
			}
		}
		...
		for (var i = 0; i < pathToExit.length; i++) {
			...
			drawRect(canvas, path.x, path.y, "rgba(255, 255, 255, 0.8)");
		}
	}
